function [c_source_file,c_other_file] = coregister( ...
    source_file, target_file, other_file, out_path, com_flag)
...
% [other_p,other_n,other_e] = fileparts(other_file);
% other_vols = cellstr(spm_select('ExtFPList',other_p,['^' other_n other_e '$'],inf));
other_vols = other_file;

% other_vols=[];
% cnt=1;
% for i=1:length(other_file)
%     for j=1:length(other_file{i})
%     other_vols{cnt,1}=other_file{i}{j};
%     cnt=cnt+1;
%     end
% end
