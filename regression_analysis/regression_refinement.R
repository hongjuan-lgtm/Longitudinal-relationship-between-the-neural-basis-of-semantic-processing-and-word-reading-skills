# Refinement: does T1 word reading predict T2 neural activity, controlling
# for T1 nonverbal IQ and T1 neural activity? Both neural measures use the T2 mask.
# Base R only; no standardization.
# Input: neural_regression_data.tsv from prepare_neural_data.R.
# Run separately for each cohort (5-7 or 7-9):
# source('regression_refinement.R')
# regression_refinement('/path/to/5-7/neural_regression_data.tsv',
#                       cohort = '5-7', output_dir = '/path/to/results/5-7')

regression_refinement <- function(data_file, cohort, output_dir) {
  ...
  variables <- c('brain_T2mask_T2beta', 'nonverbal_IQ_T1', 'brain_T2mask_T1beta',
                 'wordreading_T1')
  ...
  for (roi in c('AG','pMTG','vIFG')) for (depth in c('shallow','deep')) {
    ...
    model1 <- lm(brain_T2mask_T2beta ~ nonverbal_IQ_T1 + brain_T2mask_T1beta, data=z, na.action=na.fail)
    model2 <- lm(brain_T2mask_T2beta ~ nonverbal_IQ_T1 + brain_T2mask_T1beta + wordreading_T1,
  ...
  writeLines(c(paste('Input:',normalizePath(data_file)),paste('Cohort:',cohort),
    'Model 1: brain_T2mask_T2beta ~ nonverbal_IQ_T1 + brain_T2mask_T1beta',
    'Model 2: brain_T2mask_T2beta ~ nonverbal_IQ_T1 + brain_T2mask_T1beta + wordreading_T1',
  ...
  message('Completed six refinement analyses for ',cohort,' in ',output_dir)
  invisible(result)
}
